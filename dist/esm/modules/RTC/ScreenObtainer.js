...
const ScreenObtainer = {
    ...
    _createObtainStreamMethod() {
        if (browser.isNWJS()) {
            return (onSuccess, onFailure) => {
                window.ConfabboxNW.obtainDesktopStream(onSuccess, (error, constraints) => {
                    let confabError;
                    // FIXME:
                    ...
                    if (error && error.name === 'InvalidStateError') {
                        confabError = new ConfabTrackError(ConfabTrackErrors.SCREENSHARING_USER_CANCELED);
                    }
                    else {
                        confabError = new ConfabTrackError(error, constraints, ['desktop']);
                    }
    ...
    obtainScreenOnElectron(onSuccess, onFailure) {
        if (window.ConfabboxScreenObtainer && window.ConfabboxScreenObtainer.openDesktopPicker) {
            ...
            }, err => onFailure(new ConfabTrackError(ConfabTrackErrors.ELECTRON_DESKTOP_PICKER_ERROR, err)));
        }
        else {
