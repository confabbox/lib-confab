...
export default class ChatRoom extends Listenable {
    ...
    discoRoomInfo() {
        ...
        this.connection.sendIQ(getInfo, result => {
            ...
            const lobbyRoomField = $(result).find('>query>x[type="result"]>field[var="muc#roominfo_lobbyroom"]>value');
            if (this.lobby) {
                this.lobby.setLobbyRoomJid(lobbyRoomField && lobbyRoomField.length ? lobbyRoomField.text() : undefined);
            }
            const isBreakoutField = $(result).find('>query>x[type="result"]>field[var="muc#roominfo_isbreakout"]>value');
            ...
            const breakoutMainRoomField = $(result).find('>query>x[type="result"]>field[var="muc#roominfo_breakout_main_room"]>value');
    ...
    onPresence(pres) {
        ...
        if (from === this.myroomjid) {
            ...
            if (member.isFocus) {
                ...
                this.eventEmitter.emit(XMPPEvents.MUC_MEMBER_JOINED, from, member.nick, member.role, member.isHiddenDomain, member.statsID, member.status, member.identity, member.botType, member.jid, member.features, member.isReplaceParticipant);
            ...
            switch (node.tagName) {
                ...
                case 'transcription-status': {
                    ...
                    if (status && status !== this.transcriptionStatus) {
                        ...
                        this.eventEmitter.emit(XMPPEvents.TRANSCRIPTION_STATUS_CHANGED, status);
                    }
                    break;
                }
                case 'call-control': {
    ...
    onPresenceUnavailable(pres, from) {
        ...
        if (destroySelect.length) {
            let reason;
            const reasonSelect = $(pres).find('>x[xmlns="http://jabber.org/protocol/muc#user"]'
                + '>destroy>reason');
    ...
    onMessage(msg, from) {
        ...
        if (from === this.roomjid) {
            ...
            if ($(msg).find('>x[xmlns="http://jabber.org/protocol/muc#user"]>status[code="104"]').length) {
                ...
                this.eventEmitter.emit(XMPPEvents.INVITE_MESSAGE_RECEIVED, from, invite.attr('from'), txt, password);
            }
        }
        ...
        if (txt) {
            if (type === 'chat') {
                this.eventEmitter.emit(XMPPEvents.PRIVATE_MESSAGE_RECEIVED, from, txt, this.myroomjid, stamp);
            }
            else if (type === 'groupchat') {
                this.eventEmitter.emit(XMPPEvents.MESSAGE_RECEIVED, from, txt, this.myroomjid, stamp);
            }
        }
    }
    /**
    ...
    addAudioInfoToPresence(mute) {
        ...
        return this.addOrReplaceInPresence(audioMutedTagName, {
            value: mute.toString()
        });
    }
    /**
    ...
    addVideoInfoToPresence(mute) {
        ...
        return this.addOrReplaceInPresence(videoMutedTagName, {
            value: mute.toString()
        });
    }
    /**
