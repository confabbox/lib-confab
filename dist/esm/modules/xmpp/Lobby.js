...
export default class Lobby {
    ...
    constructor(room) {
        ...
        this.mainRoom.addEventListener(XMPPEvents.LOCAL_ROLE_CHANGED, maybeJoinLobbyRoom);
        this.mainRoom.addEventListener(XMPPEvents.MUC_MEMBERS_ONLY_CHANGED, maybeJoinLobbyRoom);
        this.mainRoom.addEventListener(XMPPEvents.ROOM_CONNECT_MEMBERS_ONLY_ERROR, jid => {
        ...
        if (isModerator) {
            ...
            this.lobbyRoom.addEventListener(XMPPEvents.MUC_DESTROYED, () => {
                ...
                Object.keys(this.lobbyRoom.members)
                    .forEach(j => this.mainRoom.eventEmitter.emit(XMPPEvents.MUC_LOBBY_MEMBER_LEFT, Strophe.getResourceFromJid(j)));
            ...
            this.mainRoom.addEventListener(XMPPEvents.INVITE_MESSAGE_RECEIVED, (roomJid, from, txt, invitePassword) => {
    ...
    approveAccess(id) {
        ...
        if (memberRoomJid) {
            ...
            const msgToSend = $msg({ to: mainRoomJid })
