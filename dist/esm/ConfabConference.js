...
import { FEATURE_E2EE, FEATURE_JIGASI, CONFAB_MUC_TYPE } from './modules/xmpp/xmpp';
...
ConfabConference.prototype._init = function (options = {}) {
    ...
    this.room = this.xmpp.createRoom(this.options.name, Object.assign(Object.assign({}, config), { statsId: this._statsCurrentId }), ConfabConference.resourceCreator);
    ...
    if (config.enableTalkWhileMuted && browser.supportsVADDetection()) {
        ...
        if (config.createVADProcessor) {
            ...
            vadTalkMutedDetection.on(DetectionEvents.VAD_TALK_WHILE_MUTED, () => this.eventEmitter.emit(ConfabConferenceEvents.TALK_WHILE_MUTED));
    ...
    if (config.enableNoisyMicDetection && browser.supportsVADDetection()) {
        if (config.createVADProcessor) {
            ...
            vadNoiseDetection.on(DetectionEvents.VAD_NOISY_DEVICE, () => this.eventEmitter.emit(ConfabConferenceEvents.NOISY_MIC));
...
ConfabConference.prototype.authenticateAndUpgradeRole = function (options) {
    return authenticateAndUpgradeRole.call(this, Object.assign(Object.assign({}, options), { onCreateResource: ConfabConference.resourceCreator }));
};
/**
 ...
 * @returns {Promise}
 */
ConfabConference.prototype.leave = function () {
...
ConfabConference.prototype.addTrack = function (track) {
    ...
    if (localTracks.length > 0) {
        ...
        if (FeatureFlags.isMultiStreamSupportEnabled() && mediaType === MediaType.VIDEO) {
            const sourceName = getSourceNameForConfabTrack(this.myUserId(), mediaType, (_a = this.getLocalTracks(mediaType)) === null || _a === void 0 ? void 0 : _a.length);
...
ConfabConference.prototype._fireAudioLevelChangeEvent = function (audioLevel, tpc) {
    ...
    if (!tpc || activeTpc === tpc) {
        this.eventEmitter.emit(ConfabConferenceEvents.TRACK_AUDIO_LEVEL_CHANGED, this.myUserId(), audioLevel);
    }
};
/**
...
ConfabConference.prototype.lock = function (password) {
    ...
    return new Promise((resolve, reject) => {
        this.room.lockRoom(password || '', () => resolve(), err => reject(err), () => reject(ConfabConferenceErrors.PASSWORD_NOT_SUPPORTED));
    });
};
/**
...
ConfabConference.prototype._maybeSetSITimeout = function () {
    ...
    if (!this.jvbJingleSession
        ...
        this._sessionInitiateTimeout = window.setTimeout(() => {
            this._sessionInitiateTimeout = null;
            Statistics.sendAnalytics(createJingleEvent(ACTION_JINGLE_SI_TIMEOUT, {
                p2p: false,
 ...
 * the same jwt.
 */
ConfabConference.prototype.onMemberJoined = function (jid, nick, role, isHidden, statsID, status, identity, botType, fullJid, features, isReplaceParticipant) {
    const id = Strophe.getResourceFromJid(jid);
    if (id === 'focus' || this.myUserId() === id) {
        return;
    }
    const participant = new ConfabParticipant(jid, this, nick, isHidden, statsID, status, identity);
    ...
    this.eventEmitter.emit(ConfabConferenceEvents.USER_JOINED, id, participant);
...
ConfabConference.prototype._onMemberBotTypeChanged = function (jid, botType) {
    ...
    if (botParticipant) {
        ...
        const id = Strophe.getResourceFromJid(jid);
        this.eventEmitter.emit(ConfabConferenceEvents.BOT_TYPE_CHANGED, id, botType);
    }
 ...
 */
ConfabConference.prototype.onMemberKicked = function (isSelfPresence, actorId, kickedParticipantId, reason, isReplaceParticipant) {
    ...
    if (isSelfPresence) {
        this.eventEmitter.emit(ConfabConferenceEvents.KICKED, actorParticipant, reason, isReplaceParticipant);
        this.leave();
        return;
    }
    const kickedParticipant = this.participants[kickedParticipantId];
    kickedParticipant.setIsReplaced(isReplaceParticipant);
    this.eventEmitter.emit(ConfabConferenceEvents.PARTICIPANT_KICKED, actorParticipant, kickedParticipant, reason);
};
/**
...
ConfabConference.prototype.onLocalRoleChanged = function (role) {
    ...
    this.eventEmitter.emit(ConfabConferenceEvents.USER_ROLE_CHANGED, this.myUserId(), role);
};
...
ConfabConference.prototype.onDisplayNameChanged = function (jid, displayName) {
    ...
    this.eventEmitter.emit(ConfabConferenceEvents.DISPLAY_NAME_CHANGED, id, displayName);
};
/**
...
ConfabConference.prototype.onRemoteTrackAdded = function (track) {
    ...
    track.addEventListener(ConfabTrackEvents.TRACK_MUTE_CHANGED, () => emitter.emit(ConfabConferenceEvents.TRACK_MUTE_CHANGED, track));
    track.isAudioTrack() && track.addEventListener(ConfabTrackEvents.TRACK_AUDIO_LEVEL_CHANGED, (audioLevel, tpc) => {
        const activeTPC = this.getActivePeerConnection();
        if (activeTPC === tpc) {
...
ConfabConference.prototype._onIncomingCallP2P = function (jingleSession, jingleOffer) {
    ...
    const usesUnifiedPlan = browser.supportsUnifiedPlan()
        && (!browser.isChromiumBased() || ((_a = this.options.config.enableUnifiedOnChrome) !== null && _a !== void 0 ? _a : true));
...
ConfabConference.prototype._acceptJvbIncomingCall = function (jingleSession, jingleOffer, now) {
    ...
    const serverRegion = $(jingleOffer)
        .find('>bridge-session[xmlns="http://jitsi.org/protocol/focus"]')
        .attr('region');
    this.eventEmitter.emit(ConfabConferenceEvents.SERVER_REGION_CHANGED, serverRegion);
    this._maybeClearSITimeout();
    Statistics.sendAnalytics(createJingleEvent(ACTION_JINGLE_SI_RECEIVED, {
        p2p: false,
        value: now
    }));
    try {
    ...
    try {
        ...
        this.statistics.startCallStats(this.jvbJingleSession.peerconnection, 'confab' /* Remote user ID for JVB is 'confab' */);
...
ConfabConference.prototype.onCallEnded = function (jingleSession, reasonCondition, reasonText) {
    ...
    if (jingleSession === this.jvbJingleSession) {
        ...
        Statistics.sendAnalytics(createJingleEvent(ACTION_JINGLE_TERMINATE, { p2p: false }));
        // Stop the stats
        if (this.statistics) {
...
ConfabConference.prototype.updateDTMFSupport = function () {
    ...
    if (somebodySupportsDTMF !== this.somebodySupportsDTMF) {
        ...
        this.eventEmitter.emit(ConfabConferenceEvents.DTMF_SUPPORT_CHANGED, somebodySupportsDTMF);
    }
};
/**
...
ConfabConference.prototype.getLocalParticipantProperty = function (name) {
    const property = this.room.presMap.nodes.find(prop => prop.tagName === `confab_participant_${name}`);
...
ConfabConference.prototype._fireIncompatibleVersionsEvent = function () {
    this.eventEmitter.emit(ConfabConferenceEvents.CONFERENCE_FAILED, ConfabConferenceErrors.INCOMPATIBLE_SERVER_VERSIONS);
};
/**
...
ConfabConference.prototype._onIceConnectionFailed = function (session) {
    ...
    if (session.isP2P) {
        ...
        Statistics.analytics.addPermanentProperties({ p2pFailed: true });
        if (this.p2pJingleSession) {
            Statistics.sendAnalyticsAndLog(createP2PEvent(ACTION_P2P_FAILED, {
...
ConfabConference.prototype._onIceConnectionEstablished = function (jingleSession) {
    ...
    if (!isNaN(this.p2pEstablishmentDuration)
        ...
        Statistics.sendAnalytics(ICE_ESTABLISHMENT_DURATION_DIFF, { value: establishmentDurationDiff });
    }
    ...
    Statistics.sendAnalyticsAndLog(createP2PEvent(ACTION_P2P_ESTABLISHED, {
        initiator: this.p2pJingleSession.isInitiator
    }));
};
/**
...
ConfabConference.prototype._setP2PStatus = function (newStatus) {
    ...
    if (newStatus) {
        ...
        this.p2pJingleSession
            .setMediaTransferActive(true, isVideoActive)
            .catch(error => {
            logger.error('Failed to sync up P2P video transfer status'
                + `(${isVideoActive})`, error);
        });
    }
    else {
    ...
    this.dtmfManager = null;
    // Update P2P status
    this.eventEmitter.emit(ConfabConferenceEvents.P2P_STATUS, this, this.p2p);
...
ConfabConference.prototype._maybeStartOrStopP2P = function (userLeftEvent) {
    ...
    if (!this.p2pJingleSession && shouldBeInP2P) {
        ...
        if (this.p2pJingleSession.isInitiator && peerCount > 1) {
            Statistics.sendAnalyticsAndLog(createP2PEvent(ACTION_P2P_SWITCH_TO_JVB));
        }
        this._stopP2PSession();
    }
};
...
ConfabConference.prototype._updateRoomPresence = function (jingleSession, ctx) {
    ...
    if (!FeatureFlags.isMultiStreamSupportEnabled()) {
        ...
        if (!(localAudioTracks === null || localAudioTracks === void 0 ? void 0 : localAudioTracks.length)) {
...
//# sourceMappingURL=ConfabConference.js.map