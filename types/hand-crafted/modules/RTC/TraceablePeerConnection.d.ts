...
export default class TraceablePeerConnection {
  ...
  id: number;
  isP2P: boolean;
  remoteTracks: Map<number, Map<MediaType, ConfabRemoteTrack>>; // TODO:
  localTracks: Map<number, ConfabLocalTrack>; // TODO:
  ...
  interop: unknown; // TODO: unknown = Interop refers to sdp-interop
  simulcast: unknown; // TODO: unknown = Simulcast refers to sdp-simulcast
  ...
  getLocalVideoTrack: () => ConfabLocalTrack | undefined;
  hasAnyTracksOfType: ( mediaType: MediaType ) => boolean;
  getRemoteTracks: ( endpointId: string, mediaType: MediaType ) => ConfabRemoteTrack[];
  getRemoteSourceInfoByParticipant: ( id: string ) => string[]; // TODO:
  getTargetVideoBitrates: () => unknown; // TODO:
  getTrackBySSRC: ( ssrc: number ) => ConfabTrack | null;
  getSsrcByTrackId: ( id: string ) => number | null;
  removeRemoteTracks: ( owner: string ) => ConfabRemoteTrack[];
  getLocalSSRC: ( localTrack: ConfabLocalTrack ) => string;
  signalingState: unknown; // TODO:
  iceConnectionState: unknown; // TODO:
  localDescription: unknown; // TODO:
  remoteDescription: unknown; // TODO:
  containsTrack: ( track: ConfabLocalTrack | ConfabRemoteTrack ) => boolean;
  addTrack: ( track: ConfabLocalTrack, isInitiator?: boolean ) => Promise<void>;
  addTrackUnmute: ( track: ConfabLocalTrack ) => Promise<boolean>;
  ...
  removeTrack: ( localTrack: ConfabLocalTrack ) => void;
  ...
  replaceTrack: ( oldTrack: ConfabLocalTrack | null, newTrack: ConfabLocalTrack | null ) => Promise<boolean>;
  removeTrackMute: ( localTrack: ConfabLocalTrack ) => Promise<boolean>;
  ...
  generateNewStreamSSRCInfo: (track: ConfabLocalTrack) => TraceablePeerConnection['localSSRCs'];
  usesUnifiedPlan: () => boolean;
