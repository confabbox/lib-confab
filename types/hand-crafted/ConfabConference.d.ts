...
import ConfabRemoteTrack from './modules/RTC/ConfabLocalTrack';
import ConfabTrack from './modules/RTC/ConfabTrack';
import Transcriber from './modules/transcription/transcriber';
import ConfabVideoSIPGWSession from './modules/videosipgw/ConfabVideoSIPGWSession';
import TraceablePeerConnection from './modules/RTC/TraceablePeerConnection';
import { MediaType } from './service/RTC/MediaType';

export default class ConfabConference {
  constructor( options: {
    name: string;
    config: {
      avgRtpStatsN?: number,
  ...
  getName: () => string;
  getConnection: () => ConfabConnection;
  ...
  getLocalTracks: ( mediaType: MediaType ) => ConfabLocalTrack[];
  getLocalAudioTrack: () => ConfabLocalTrack | null;
  getLocalVideoTrack: () => ConfabLocalTrack | null;
  getPerformanceStats: () => unknown | null; // TODO:
  on: ( eventId: ConfabConferenceEvents, handler: (...args: any[]) => unknown ) => void; // TODO:
  off: ( eventId: ConfabConferenceEvents, handler: (...args: any[]) => unknown ) => void; // TODO:
  addEventListener: ( eventId: ConfabConferenceEvents, handler: (...args: any[]) => unknown ) => void; // TODO:
  removeEventListener: ( eventId: ConfabConferenceEvents, handler: (...args: any[]) => unknown ) => void; // TODO:
  ...
  addTrack: ( track: ConfabLocalTrack ) => Promise<ConfabLocalTrack>;
  onLocalTrackRemoved: ( track: ConfabLocalTrack ) => void;
  removeTrack: ( track: ConfabLocalTrack ) => void;
  replaceTrack: ( oldTrack: ConfabLocalTrack, newTrack: ConfabLocalTrack ) => Promise<unknown | ConfabTrackError>;
  getRole: () => string;
  ...
  getParticipants: () => ConfabParticipant[];
  getParticipantCount: ( countHidden?: boolean ) => number;
  getParticipantById: ( id: string ) => ConfabParticipant;
  ...
  onRemoteTrackAdded: ( track: ConfabRemoteTrack ) => void;
  ...
  onRemoteTrackRemoved: ( removedTrack: ConfabRemoteTrack ) => void;
  ...
  getSsrcByTrack: ( track: ConfabTrack ) => number | undefined;
